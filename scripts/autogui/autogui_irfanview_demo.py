...
db_conn = sql.connect('/home/thollwed/Dokumente/Berufsschule/Mittelstufenprojekt/NSDD/data/MS1.db')
...
def set_path(path_values):
    ...
    #path = r"C:\test\iview456g_x64_setup.exe"
    return path


def set_prog_dialog(name_values):
    prog_dialog = name_values[0][1]
    #prog_dialog = 'IrfanView 64-bit Setup'
    return prog_dialog
...
def set_button_list(button_values):
    ...
    print(button_list)
    #button_list = ['Weiter >', 'Weiter >', 'Weiter >', 'Weiter >', ['IrfanView FAQs (Häufig gestellte Fragen) Webseite anzeigen', 'IrfanView starten', 'Fertig stellen']]
    return button_list
...
if __name__ == '__main__':
    main()
""" Diese Funktion soll ein Benutzen von Adminrechten erzwingen, allerdings ist eine manuelle Bestätigung nötig, außerdem startet das Setup dann nicht korrekt    
    if is_admin():
        main()
    else:
